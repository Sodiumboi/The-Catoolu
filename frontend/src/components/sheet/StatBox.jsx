...
export default function StatBox({ label, sublabel, value, editable = false, onChange, compact = false }) {
  ...
  return (
    <div className={`flex flex-col items-center border border-(--border-main) bg-(--bg-input) ${compact ? 'gap-0.75 py-1.5 px-2 rounded-lg min-w-13.5' : 'gap-0.5 py-1 px-2.5 rounded-[10px] min-w-16'}`}>
      {label && (
      ...
      {editable ? (
        <input
          ...
          className={`outline-none! font-bold font-serif text-center bg-transparent border-none border-b-2 border-(--border-input) text-(--text-primary) p-0 [-moz-appearance:textfield] ${compact ? 'w-10.5 h-8 text-lg' : 'w-13 h-8 text-[22px]'}`}
          onFocus={e => e.target.style.borderBottomColor = 'var(--accent)'}
          onBlur={e  => e.target.style.borderBottomColor = 'var(--border-input)'}
        />
      ) : (
        <div className={`font-bold font-serif text-(--text-primary) leading-none ${compact ? 'text-lg pt-1 px-0 pb-0.5' : 'text-[22px] pt-0.5 px-0 pb-0.5'}`}>
          {num}
        </div>
      )}
